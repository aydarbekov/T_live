...
# with open('old_budget_10.csv', 'w', newline='') as file:
...
with open('../../data/minjust.csv', 'r', newline='') as file:
    ...
    for row in islice(rows,95828, 100000):
        ...
        zapros = load_link(f'https://oldbudget.okmot.kg/inn_income?code={inn}')
        ...
        if len(trs[1:-1]) != 0:
            for tr in trs[1:-1]:
                ...
                print(doc_num.text)
                print(date.text)
                if inn == '00708199510277' and date.text == '29.04.2013':
                    continue
                print(payer)
                print(dest)
                print(sum.text)

                with open('old_budget_10.csv', 'a+', newline='') as file:
                    writer = csv.writer(file, delimiter='|')
                    writer.writerow([inn, doc_num.text, date.text, payer,
                                     dest, sum.text])
