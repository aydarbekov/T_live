...
with open('error5.csv', 'r', newline='') as file:
    ...
    for row in islice(rows, 0, None):
        try:
            ...
            if row[0] == 'ИНН' or row[0] == '' or row[0] == 'сведений нет':
                continue
            print(row[0])
            if len(row[0]) != 0:
                dif = 14 - len(row[0])
            ...
            print(inn)
            inn = inn.strip()
            print(f'https://budget.okmot.kg/ru/income/get-inn?inn={inn}&dateFrom=2018-01-01&dateTo=2020-12-04')
            zapros = load_link(f'https://budget.okmot.kg/ru/income/get-inn?inn={inn}&dateFrom=2018-01-01&dateTo=2020-12-04')
